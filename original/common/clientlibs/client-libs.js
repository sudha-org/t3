            ...
            _faqlinkclick: function(e) {
                 try {
                    ...
                    if (_this.parent("dd.accordion-navigation.active").length == 0) {
                        (_faqslist.length == 1 && _faqslist[0] == "") ? _faqslist[0] = _faqslistitem 
                        : _faqslist.push(_faqslistitem);
        ...
        */
        ...
        window.dlganalytics = {
            ...
            setVistitorID: function() {
                ...
                s.events = s.events + ",event25=" + _satellite.getVar("Page Load Time");
            },
            ...
            trackSearch: function() {
                ...
                $("input.gsc-search-button").click(function(e) {
                    trackgoogle();
                });
                /*On page load*/
            ...
            mainNavigationClick: function(me) {
                try {
                    ...
                    navcomponent = _satellite.getVar("Page Name") + " | " + parentnav.attr("data-analytics-uci"),
                        isDesktop = parentnav.is('div'),
                        parentobj = isDesktop ? _this.parents('ul') : _this.parents("ul").find("a.selected"),
                        subnav = parentobj.length > 0 ? (isDesktop ? parentobj.attr('id') : parentobj.text()) : "";
                    navcomponent = (parentobj.length > 0 ? navcomponent + (typeof(subnav) !== "undefined" ? subnav : "") : navcomponent) + ' : ' + _this.text().trim();
                    navcomponent = navcomponent.replace(/\r?\n|\r/g, ' ').trim();
            ...
            trackContentGetQuotes: function() {
                var getquotes = $(".content-container > div:eq(3)").find("a:contains('quote')");
                if (getquotes.length > 0) {
                    /*carousel, slick carousel*/
                    var alreadytrackedlinks = $(".carousel div a, .carousel div button, .cover-levels-overview .cover-type a,#
                        homepage_container a, div# menu - wrapper - mobile - up a[href], aside.off - canvas - navigation.hide -
                        for -large - up a[href]
                        ");

                        var headertext = function(me) {
                            var _txt = $(me).parents("section").find("h1");
                            _txt = _txt.length > 0 ? _txt : $(me).parents("section").find("h2");
                            if (_txt.length > 0) {
                            ...
                            var _hdr = headertext(this),
                                ...
                                _component = _pgn + " | " + (typeof(_hdr) !== "undefined" ? (_hdr + " : ") : "") + $(this).text();
